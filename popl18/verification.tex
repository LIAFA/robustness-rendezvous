...
the synchronous semantics ``simulates''~\footnote{We refer to the standard notion of (stuttering) simulation where (sequences of) transitions in $\mathcal{S}$ are mapped to transitions of $\mathcal{S'}$.} all the borderline violations of $\mathcal{S}$. 
...
\begin{theorem}
Let $e=e_1\cdot \send{i}{p,q,v}\cdot e_2\cdot \rec{i}{q,v}$ be a borderline violation to $k$-synchronizability of $\mathcal{S}$. Then, $\synchExec{\mathcal{S'}}{k}$ contains a permutation of the following execution $e'$ (modulo a substitution of message identifiers): 
\begin{align*}
...
\subsection{Excluding Executions Violating the FIFO Semantics}
