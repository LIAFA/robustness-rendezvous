...
\begin{itemize}
	\item since the set of synchronous executions is prefix-closed, it is enough to look for \emph{borderline} violations, i.e., executions which
are not synchronous but for which every strict prefix is synchronous,
	\item starting from the original system $\mathcal{S}$, define a new system $\mathcal{S'}$ whose set of executions under
the synchronous semantics contains all the borderline synchronizability violations of $\mathcal{S}$, 
	\item define a monitor $\mathcal{M}_{\mathit{fifo}}$, which identifies executions of $\mathcal{S'}$ under the synchronous semantics
which are not executions of the original system $\mathcal{S}$ ($\mathcal{M}_{\mathit{fifo}}$ goes to an error state whenever it finds such 
an execution), 
...
($\mathcal{M}_{\mathit{viol}}$ goes to an error state whenever it finds such an execution),
	\item establish that $\mathcal{S}$ is $k$-synchronizable whenever $\mathcal{S}\paral \mathcal{M}_{\mathit{fifo}}\paral \mathcal{M}_{\mathit{viol}}$
doesn't reach an error state (more precisely, none of the two monitors $\mathcal{M}_{\mathit{fifo}}$ and $\mathcal{M}_{\mathit{viol}}$ goes to an error state).
\end{itemize}
...
$e$ is $k$-synchronous. We define a system $\mathcal{S'}$ whose synchronous semantics admits all the borderline violations of 
$\mathcal{S}$, if any, where essentially, the reception of exactly one message is delayed (w.r.t. the synchronous semantics).
...
Formally, the system $\mathcal{S'}$ is defined by $\mathcal{S'}=((\<Lsts>_p,\delta'_p,l_p^0)|p\in\<Pids>\cup\{\pi\})$ where
\begin{itemize}
...
We first show that $\synchExec{\mathcal{S'}}{k}$ contains all the borderline violations of $\mathcal{S}$. An experienced reader may notice that
...
earlier (which violate the FIFO semantics of the message buffers in $\mathcal{S}$). We show how to exclude such scenarios later in this section.
...
The following result shows that indeed $\synchExec{\mathcal{S'}}{k}$ contains all the borderline violations of $\mathcal{S}$, modulo a substitution of the message identifiers. 
...
\begin{theorem}
Let $e$ be a borderline violation to $k$-synchronizability of $\mathcal{S}$. Then, $e\in \synchExec{\mathcal{S'}}{k}$ (modulo a substitution of message identifiers).
\end{theorem}
\begin{proof}
Let $e=e_1\cdot s\cdot e_2\cdot r$ be a borderline violation to $k$-synchronizability of $\mathcal{S}$, where $s\match r$. 
Since $e_1\cdot s\cdot e_2$ is $k$-synchronous, there exists a conflict-preserving permutation $e_1'\cdot s\cdot e_2'$ of $e_1\cdot s\cdot e_2$ which belongs to $\synchExec{\mathcal{S'}}{k}$ (modulo a substitution of message identifiers). The execution $e_1'\cdot s\cdot e_2'$ is obtained through a sequence of \textsc{$k$-exchange} transitions. We show 

 $e_1''\cdot s\cdot e_2''$ 

Let $s'$ be a send action in $e_1$ (occurring before $s$ in $e$) matching a receive $r'$ in $e$. We first show that there exists a conflict-preserving permutation of $e_1\cdot s\cdot e_2$ where $r'$ occurs before $s$ as well. Assuming by contradiction that such a permutation doesn't exist, we get that $s'\leadsto s$ and $s\leadsto r'$. 

 Assuming by contradiction that this is not the case, 

$e_1'\cdot s\cdot e_2'$  in which every message sent 
during $e_1'$ is received during $e_1'$ as well. 

receive action matching a send action in $e_1$ is also contained in $e_1$. 


Since $e$ is a borderline violation, w.l.o.g., we assume that $e_1\cdot s\cdot e_2$ is $k$-synchronous. 


This implies that for every send action in 
$e_1$, the matching receive action is  contained also in $e_1$. Assume by contradiction that this is not the case. Then, 
\end{proof}
...
Let $e$ be a borderline $k$-synchronizability violation: 
\begin{align*}
e=e_1\cdot \send{1}{p_1,q_1,\_} \cdot e_2\cdot \rec{1}{q_1,\_}
\end{align*} 
